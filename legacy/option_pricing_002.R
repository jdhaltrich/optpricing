...
instrument_code_init<-"'AAPL200731C00405000'"
...
instrument<-dbGetQuery(option_database,
						           paste(
											"select * from options.extract_instrument_analysis_2 where instrument_code=",
...
future_projection_days<-dbGetQuery(option_database,
												paste("select fecha_id from options.master_operaciones where fecha_id>(select max(effective_date) from options.extract_instrument_analysis_2 where instrument_code=",
														instrument_code_init,
														")",
														"and fecha_id<=(select min(expiration_date) from options.extract_instrument_analysis_2 where instrument_code=",
														instrument_code_init,
														") and cboe=1"
														)
									)
...
write.xlsx(
            cbind(
              ...
              ),'D:/a/inversion/run_results/202008/20200810/AAPL200731C00405000.xlsx',
			  sheetName="results",
			  col.names=TRUE,
			  row.names=TRUE,
			  append=FALSE,
