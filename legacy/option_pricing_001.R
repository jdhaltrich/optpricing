	...
	instrument <- dbGetQuery(option_database,
							 "select * from options.extract_instrument_analysis_2
								where instrument_code='AAPL200731C00405000'
	...
	future_projection_days <- dbGetQuery(option_database,
											"select fecha_id
											from options.master_operaciones
											where fecha_id>(select
															max(effective_date)
															from options.extract_instrument_analysis_2
															where instrument_code='AAPL200731C00405000'
															)
												and fecha_id<=(select
																min(expiration_date)
...
#                          ifelse(
#                                  (instrument[,"effective_date"])<=expiration_date,
#                                    1,
#                                    0
#                                  ),
#                          prices_length-parameter_days_mean
...
#delta_t_year <- unlist(
#                        lapply(
#                                1:(prices_length-parameter_days_mean+remaining_days),
...
#							(input_volatility,delta_t_year,s,k,r,call_real
...
write.xlsx(
            cbind(
              ...
              ),'D:/a/inversion/run_results/202008/20200805/AAPL200731C00405000_2.xlsx',
			  sheetName="results",
			  col.names=TRUE,
			  row.names=TRUE,
			  append=FALSE,
