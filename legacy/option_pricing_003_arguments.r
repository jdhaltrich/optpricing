...
instrument<-dbGetQuery(option_database,
						           paste(
											"select * from options.extract_instrument_analysis_3 where instrument_code=",
...
future_projection_days<-dbGetQuery(option_database,
												paste("select fecha_id from options.master_operaciones where fecha_id>(select max(effective_date) from options.extract_instrument_analysis_3 where instrument_code=",
														instrument_code_init,
														")",
														"and fecha_id<=(select min(expiration_date) from options.extract_instrument_analysis_3 where instrument_code=",
														instrument_code_init,
														") and cboe=1"
														)
									)
...
delta_t_year <- unlist(
						lapply(1:(prices_length-parameter_days_mean+remaining_days),
...
black_scholes_strip <- mapply(
                              black_scholes,
                              s_actual,volatility_actual,remaining_time_actual,strike_final,.03
                              )
...
greek_gamma_matrix <- matrix(
                              unlist(greek_gamma_strip),
                              ncol=(prices_length-parameter_days_mean)
                              )
...
greek_vega_matrix <- matrix(
                            unlist(greek_vega_strip),
                            ncol=(prices_length-parameter_days_mean)
                            )
...
implied_volatility_strip <- mapply(
                                    implied_volatility,
                                    initial_volatility,remaining_time_actual,s_actual,strike_final,.03,tail(instrument[,"price"],prices_length-parameter_days_mean)
                                    )
...
probability_strip <- mapply(
                            probability,
                            405,s_actual,volatility_actual,remaining_time_actual,mean_log_variation
                            )
...
#  sample_path <- cumprod(c(underlying,exp((media_variacion-.5*(volatility_simulation)^2)*delta_t_simulation+change*volatility_simulation*(delta_t_simulation^(.5)))))
...
#sum(simulacion_precios_2[5,])/length(simulacion_precios_2[5,])
 ...
 write.csv(
            cbind(
              ...
              ),"/laynestaley88/inversion/modelo/project_code/R_CODE/black_scholes_pricing/output_prueba.csv",
			  )
