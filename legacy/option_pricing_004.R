...
instrument_actuals_last <- instrument_actuals[length(s_actual),]

fields_actuals <- colnames(instrument_actuals)

exprt_output_layout_actuals <- cbind(
				instrument_actuals,
				volatility_actual,
				s_actual,
				mean_log_variation,
...
proj_type_projected <- rep("Projected_Valuation",length(remaining_time_to_expiration))

instrument_projected <- c(proj_type_projected,rep(1,length(proj_type_projected))) 
...
mc_options <- function(mc_sim,mc_s0,mc_sigma,mc_delta_t,mc_K,mc_r) {
	init_time <- Sys.time()
...
#write.csv(export_output_layout,
